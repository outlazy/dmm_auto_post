...
def fetch_name_and_size_from_detail_page(url, item):
    ...
    # 名前にすでに年齢が含まれている場合（例: "みなみ(21)"）は除去して検索
    base_name = re.sub(r'\(\d+\)', '', name).strip()

    if not size_str:
        profile = search_actress_profile(base_name, API_ID, AFF_ID)
        # 別名でも見つからない場合、元の名前で再試行
        if not profile and base_name != name:
            print(f"  元名前で再検索: {name}")
            profile = search_actress_profile(name, API_ID, AFF_ID)
        if profile:
